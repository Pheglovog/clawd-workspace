...
3. **自定义授权**
   ...
   modifier onlyCustomAuthorized() {
       if (msg.sender != owner() && !_customAuthorized[msg.sender]) 
...
function updateCarInfo(
    ...
    emit CarInfoUpdated(msg.sender, tokenId, oldMileage, mileage, oldCondition, condition);
}
```
...
function calculateFee(uint256 amount, uint256 feeRate) public pure returns (uint256) {
    return Math.mulDiv(amount, feeRate, 10000);  // 精确计算 0.01% 费用
}
```
...
function batchUpdate(uint256[] calldata tokenIds, uint256[] calldata mileages) public onlyCustomAuthorized {
    ...
    unchecked {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            emit CarInfoUpdated(tokenIds[i], mileages[i], _carInfos[tokenIds[i]].condition);
        }
    }
}
```
